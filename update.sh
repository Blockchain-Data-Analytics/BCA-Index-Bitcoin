...
if [ ${UPDATE_HEIGHT} -ge ${LAST_BTC_BLOCK} ]; then
...
if [ ${TXCOUNT} -lt ${CHUNK_LENGTH} ]; then
    echo "txcount $TXCOUNT is less than minimally expected ${CHUNK_LENGTH}"
    exit 1
fi
...
if [ ${BLCOUNT} -ne ${CHUNK_LENGTH} ]; then
    echo "txcount $BLCOUNT is not equal expected ${CHUNK_LENGTH}"
    exit 1
fi
...
if [ ${BLCOVERAGE} -ne ${CHUNK_LENGTH} ]; then
    echo "txcount $BLCOVERAGE is not equal expected ${CHUNK_LENGTH}"
    exit 1
fi
...
if [ ${TXCOVERAGE} -ne ${CHUNK_LENGTH} ]; then
    echo "txcount $TXCOVERAGE is not equal expected ${CHUNK_LENGTH}"
    exit 1
fi
